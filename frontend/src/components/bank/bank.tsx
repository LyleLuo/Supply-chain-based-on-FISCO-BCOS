...
const Bank: React.FunctionComponent = () => {
  ...
  const post = () => {
    fetch(postUrl, {
      ...
      body: JSON.stringify({ funcName:"create_bill", funcParam: [title,title2,money], text: text })
  ...
  const post2 = () => {
    fetch(postUrl, {
      ...
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ funcName: "finance", funcParam: [man,getmoney], text: text })
    }).then(res => res.json()).then(data => {
      //console.log(data);
      //console.log(data.result);
      console.log(data.result);
  ...
  const BeforePost =
    <Stack>
      <Stack.Item>
        <Text variant="xxLarge">银行</Text>
      </Stack.Item>
      <br></br>
      <Stack.Item>
        <Text variant="large">签发账单</Text>
      </Stack.Item>
      <Label>让两个用户之间建立债务关系</Label>
      ...
      /> */}
      <Stack.Item styles={{ root: { paddingTop: 10, width: 300 } }}>
        <TextField label="债权人地址" onChange={(_, v) => setTitle(v)} />
      </Stack.Item>
      <Stack.Item styles={{ root: { paddingTop: 10, width: 300 } }}>
        <TextField label="债务人地址" onChange={(_, v) => setTitle2(v)} />
      </Stack.Item>
      <Stack.Item styles={{ root: { paddingTop: 10, width: 300 } }}>
        <TextField label="债务金额" onChange={(_, v) => setMoney(v)} />
      </Stack.Item>
      ...
      <Stack.Item styles={{ root: { paddingTop: 10, width: 300 } }}>
        <PrimaryButton text="签发账单" onClick={post} />
      </Stack.Item>
      ...
      <br></br>
      <Stack.Item>
        <Text variant="large">融资</Text>
      </Stack.Item>
      <Label>融资金额需小于融资者的总放债额度，融资后公司资产增加相应数值</Label>
      <Stack.Item styles={{ root: { paddingTop: 10, width: 300 } }}>
        <TextField label="融资人地址" onChange={(_, v) => setMan(v)} />
      </Stack.Item>
      <Stack.Item styles={{ root: { paddingTop: 10, width: 300 } }}>
        <TextField label="融资金额" onChange={(_, v) => setGetmoney(v)} />
      </Stack.Item>
      ...
      <Stack.Item styles={{ root: { paddingTop: 10, width: 300 } }}>
        <PrimaryButton text="融资" onClick={post2} />
      </Stack.Item>
      {

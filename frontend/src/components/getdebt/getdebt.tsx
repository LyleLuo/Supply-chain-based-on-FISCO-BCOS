...
const Getdebt: React.FunctionComponent = () => {
  ...
  const post = () => {
    fetch(postUrl, {
      ...
      setMan(data.result[0].data);
      setcount(data.result[1].data);
  ...
  const post2 = () => {
    fetch(postUrl, {
      ...
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ funcName: title2, funcParam: tags, text: text })
    }).then(res => res.json()).then(data => {
      //console.log(data);
      //console.log(data.result);
      console.log(data.result[0].data);
      setcount2(data.result[0].data);
      setTags([]);
      setType(data.result);
    });
  };
  ...
  const options: IDropdownOption[] = [
    { key: 'getDebt', text: '查看欠条' },
    { key: 'getReceipt', text: '查看收据' },
  ];
  const options2: IDropdownOption[] = [
    { key: 'get_total_debt', text: '查看负债' },
    { key: 'get_total_receipt', text: '查看放债' },
  ];

  const BeforePost =
    <Stack>
      <Stack.Item>
        <Text variant="xxLarge">查看收据或欠条</Text>
      </Stack.Item>
      <br></br>
      <Stack.Item>
        <Text variant="large">按id查看收据或欠条</Text>
      </Stack.Item>
      ...
      <Stack.Item styles={{ root: { paddingTop: 20, paddingLeft: 0 } }}>
            <Label>返回结果：</Label>
            <Label>债权人/债务人地址：{man}</Label>
            <Label>金额：{count}</Label>
            {/* {
              results.map(
                (result, index) => {
      ...
      <Stack.Item styles={{ root: { paddingTop: 10, width: 300 } }}>
        <PrimaryButton text="按id查看" onClick={post} />
      </Stack.Item>
      ...
      <Stack.Item styles={{ root: { paddingTop: 20, paddingLeft: 0 } }}>
            <Label>返回结果：</Label>
            <Label>总金额：{count2}</Label>
            {/* {
              results2.map(
                (result2, index) => {
                  return <i key={index}>{" " + JSON.stringify(result2)}</i>;
                }
              )
            } */}
      </Stack.Item>
      ...
      <Stack.Item styles={{ root: { paddingTop: 10, width: 300 } }}>
        <PrimaryButton text="查看总数" onClick={post2} />
      </Stack.Item>
      {

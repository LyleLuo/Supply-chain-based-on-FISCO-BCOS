...
const Post: React.FunctionComponent = () => {
  ...
  const options: IDropdownOption[] = [
    { key: 'bank_info', text: 'bank_info' },
    { key: 'register_company', text: 'register_company' },
    { key: 'getAsset', text: 'getAsset'},
    { key: 'getDebt', text: 'getDebt' },
    { key: 'getReceipt', text: 'getReceipt' },
    { key: 'get_total_debt', text: 'get_total_debt' },
    { key: 'get_total_receipt', text: 'get_total_receipt' },
    { key: 'create_bill', text: 'create_bill' },
    { key: 'transfer_bill', text: 'transfer_bill' },
    { key: 'finance', text: 'finance' },
    { key: 'settle', text: 'settle' },
  ];

  const BeforePost =
    <Stack>
      <Stack.Item>
        <Text variant="xxLarge">调用合约</Text>
      </Stack.Item>
      <Dropdown
        placeholder="选择合约"
        label="选择需要调用的合约"
      ...
      <Stack.Item>
        <Text variant="large">合约使用说明</Text>
      </Stack.Item>
      ...
      <Label>8、create_bill，签发账单，只有银行账户可以操作，让两个用户之间建立债务关系。需要三个参数，分别为(1)债权人的地址(2)债务人的地址(3)借款数额。无返回值</Label>
      ...
      <Stack.Item styles={{ root: { paddingTop: 10, width: 300 } }}>
        <PrimaryButton text="运行合约" onClick={post} />
      </Stack.Item>
      {
